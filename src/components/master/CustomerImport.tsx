...
export function CustomerImport({ onImportDone, companyId }: CustomerImportProps) {
  ...
  const downloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Modèle_Clients");
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (count > 0) {
          onImportDone();
          toast.success(`${count} client(s) importé(s) avec succès.`);
        }
      } catch (err: any) {
            ...
            {imported > 0 && (
              <div className="bg-success/10 border border-success/20 rounded-md p-3 flex items-start gap-3">
                ...
                <div className="text-sm text-success font-medium">
                  {imported} client(s) importé(s) avec succès.
                </div>
              </div>
            )}
